...
test("Handle Dialogs", async ({ page }) => {
	// **1. Handling prompt() Dialogs (Auto-Dismissal by Default):**
  ...
  `);
	...
	// **2. Handling beforeunload Dialog:**

	page.on("dialog", async (dialog) => {
		assert(dialog.type() === "beforeunload"); // Verify it's a beforeunload dialog
		await dialog.dismiss(); // Dismiss the dialog
	});
	// Close the page, triggering the beforeunload handler
